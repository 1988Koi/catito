...
//Nao tem mais gracas a Deus consegui implementar um sistema de .txt
...
window.voltarclick = voltarclick;
...
const selectedMusic = musicSelect.value;
  ...
  try {
    const response = await fetch(`notes/${selectedMusic}.txt`);
    ...
    const notas = linhas.map(linha => {
      try {
        return Function('"use strict";return (' + linha + ')')();
      } catch (e) {
    ...
    if (!Array.isArray(notas) || notas.length === 0) {
      throw new Error("Mapa vazio ou inválido");
    }

    noteMaps[selectedMusic] = notas; 
    currentSongTotalNotes = notas.length;
    console.log(`Mapa de notas carregado: ${currentSongTotalNotes} notas.`);

    audio = new Audio(`music/${selectedMusic}.mp3`); 
    audio.currentTime = 0;
    audio.addEventListener('ended', handleSongEnd);

    notas.forEach(noteInfo => {
      if (noteInfo && typeof noteInfo.time === 'number' && typeof noteInfo.lane === 'number') {
        setTimeout(() => {
         ...
         console.warn("Nota inválida:", noteInfo);
      }
    });
    ...
    console.error("Erro ao carregar o mapa de notas:", error);
    alert(`Erro ao carregar o mapa de notas para "${selectedMusic}". Verifique se o arquivo notes/${selectedMusic}.txt existe e está formatado corretamente.`);
  }
}

//==================================
// Function do fim da musica
//==================================
