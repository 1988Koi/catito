...
let combo = 0;
let score = 0;
let totalAccumulatedPoints = parseInt(localStorage.getItem('totalAccumulatedPoints')) || 10000;
let activeNotes = [];
...
function checkHit(lane) {
  activeNotes.forEach(note => {
    ...
    if (note.lane === lane && top > hitWindowBottom && top < hitWindowTop && !note.hit) {
      ...
      if (rankDisplayElement) rankDisplayElement.textContent = currentSongHits;
...
function startGame() {
  ...
  if (chart) {
    ...
    chart.forEach(note => {
      setTimeout(() => {
        const noteElement = spawnNote(note.lane);
        activeNotes.push({ el: noteElement, lane: note.lane, hit: false });
      }, note.time);
    });
  }
...
function handleSongEnd() {
  ...
  if(rankDisplayElement) {
    ...
    rankDisplayElement.className = `rank-${rank}`;
    rankDisplayElement.style.display = 'block';
  }
}
...
function unlockAchievement(id) {
  ...
  alert(`Conquista desbloqueada: ${achievements[id].name} - ${achievements[id].description}`);
}